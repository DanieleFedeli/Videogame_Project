...
int Packet_handler_tcp(char* PACKET, char* SEND, void* arg){
	...
	PacketHeader* header = (PacketHeader*)PACKET;
<<<<<<< HEAD
	if(DEBUG)printf("%sSpacchetto...\tHeadertype = %d\n",TCP, header->type);
=======
	if(DEBUG)printf("%sSpacchetto...\n",TCP);
>>>>>>> 95e9c4673c828aa80f9f2ee8b6591352408ed724
...
<<<<<<< HEAD
		...
		if(DEBUG)printf("%sSize header: %d\n", TCP, header->size);
		ImagePacket* img_packet = (ImagePacket*)Packet_deserialize(PACKET, header->size);
		
		if(img_packet->id < 0){
			if(DEBUG)printf("%sErrore id\n", TCP);
=======
...
<<<<<<< HEAD
	...
	while(shouldCommunicate && shouldThread && count <3){
		char RECEIVE[BUFFERSIZE], SEND[BUFFERSIZE];
...
=======
	...
	/** QUESTO CICLO SERVE PER LEGGERE COSA VUOLE IL CLIENT, PROCESSARE LA RISPOSTA ED INVIARLA **/
	while(shouldCommunicate && shouldThread){
		bytes_read = 0;
		
		/** AZZERO BUFFER PER ELIMINARE BYTES NON NECESSARI**/
		...
		if(bytes_read < 0) shouldThread = 0;
>>>>>>> 95e9c4673c828aa80f9f2ee8b6591352408ed724
		if(DEBUG)printf("%sStringa di %d bytes\n",TCP, bytes_read);

		/** CHIAVO UN HANDLER PER GENERARE LA RISPOSTA**/
		msg_len = Packet_handler_tcp(RECEIVE, SEND, arg);
<<<<<<< HEAD
		//ERROR_HELPER(msg_len, "Errore nella spacchettazione");
	
		/** INVIO RISPOSTA AL CLIENT**/
		if(msg_len < 1) continue; //Previene errori in caso di errato spacchettamento o di PostTexture
	...
	if(DEBUG) printf("%sDistruggo utente con id %d\n", TCP, socket);
...
=======
		...
		if(DEBUG)printf("%sStringa inviata con successo\n",TCP);
...
<<<<<<< HEAD
		...
		argTcp.arg = args;
		argTcp.idx = new_sock;
		if(DEBUG)printf("%sSocket in accept: %d\n", SERVER, new_sock);
